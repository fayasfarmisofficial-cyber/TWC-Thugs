...
8. In the REPL the banner reads `○ model off`; type `/key add`, pick a provider (Anthropic key, or Ollama with no key), and it flips to `● model on`. Then: `what breaks if I rename classify_consumers?` — the assistant answers through `graph_search`/`amu_map` and proposes the next amu command; `amu watch --phase 1` in a second pane shows nodes flip to `↯` as you edit.
